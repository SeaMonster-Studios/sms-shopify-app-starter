...
let query = (q, ast) => {
  open GraphQL
  Client.instance.query->Obj.magic({"query": ast, "variables": q["variables"]})
    |> then_(response => {
...
let handler: handler<event<requestHeaders>> = (event, _context) => make((~resolve, ~reject as _) =>
    ...
    | Ok(verified) =>
      if verified.isInstalled {
        getShop(verified.shop)->FutureJs.fromPromise(error => {
          ...
          | Ok(Some(data)) =>
            ...
            | None => Future.make(r => "Failed to fetch shop after verification"->Error->r)
            | Some(shop) => Future.make(r => shop->Ok->r)
            }
          | Ok(None) | Error(_) =>
            Future.make(r => "Failed to fetch shop after verification"->Error->r)
          }
        )->Future.get(res =>
          switch res {
          | Ok(data) =>
            ...
            resolve(.{
              ...
              headers: headersBase(
                ~location=j`${verified.env.appUrl}?accessToken=$accessToken&shop=$shop&apiKey=$apiKey`,
                (),
              ),
            })
          | Error(message) =>
        ...
        resolve(.{
          ...
          body: Js.Nullable.null,
          headers: headersBase(
            ~location=j`https://$shop/admin/oauth/authorize?client_id=$apiKey&scope=$scopes&redirect_uri=$redirectUri&state=$nonce`, // $shop/admin/oauth/authorize?client_id=$apiKey&scope=$scopes&redirect_uri=$redirectUri&state=$nonce|j},
            (),
          ),
        })
      }
