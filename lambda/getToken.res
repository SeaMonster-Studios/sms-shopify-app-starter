...
type queryParamsForCheck = {"shop": string, "code": string, "state": string, "timestamp": string}
...
let mutation = (q, ast) => {
  open GraphQL
  Client.instance.mutate->Obj.magic({"mutation": ast, "variables": q["variables"]})
...
let handler: handler<event<requestHeaders>> = (event, _context) => make((~resolve, ~reject as _) =>
    ...
    | Ok(verified) =>
      ...
      | Some(code) =>
        ...
        Fetch.fetchWithInit(
          j`https://$shop/admin/oauth/access_token`, // $shop/admin/oauth/access_token|j},
        ...
        ->Future.mapError(error => {
          ...
          j`Failure to retrieve access token with error: $errorString`
        })
        ->Future.flatMapOk(response =>
          response->Fetch.Response.json->FutureJs.fromPromise(error => {
            error->Sentry.capturePromiseException
            error->Js.log
            error
        ...
        ->Future.flatMapOk(({access_token, scope}) =>
          if scope == verified.env.scopes {
            createShop(shop, access_token)->FutureJs.fromPromise(error => {
              error->Sentry.capturePromiseException
              error->Js.log
              error
            })->Future.mapError(error => {
              let error: string = error->Obj.magic
              j`Failed to store shop: $error`
            })->Future.mapOk((_): Types.shopifyAuthed => {
              shop: shop,
            ...
            Future.make(r =>
              j`Scope validation failed: scopes from token ($scope) does not match rhe requested scopes ($scopes)`
              ->Error
              ->r
            )
          }
        ...
        ->Future.get(res =>
          ...
          | Ok({accessToken, shop, apiKey}) =>
            resolve(.{
              statusCode: 302,
              headers: headersBase(
                ~location=j`${appUrl->Belt.Option.getWithDefault(
                  "",
                )}?accessToken=$accessToken&shop=$shop&apiKey=$apiKey`,
                (),
              ),
      ...
      | None =>
        resolve(.{
          statusCode: 400,
          body: "`code` query parameter was not provided. Get token request cannot be completed."->Js.Nullable.return,
          headers: headers,
        })
      }
    }
