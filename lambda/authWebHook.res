...
let handler: handler<event<requestHeaders>> = (event, _context) => {
  open Js.Promise
  make((~resolve, ~reject as _) =>
    ...
    | Some(accessToken) =>
      resolve(.{
        statusCode: 200,
        body: {
          "X-Hasura-Admin-Secret": hasuraAdminSecret->Option.getWithDefault(""),
          "X-Hasura-Role": "shop",
    ...
    | None => resolve(.{statusCode: 400, body: Js.Nullable.null, headers: headers})
    }
  ) |> catch(error =>
    Js.Promise.resolve({
      statusCode: 500,
