...
module Inner = {
  @react.component
  let make = () => {
    let (simple, _full) = ApolloHooks.useQuery(ShopQueries.GetAuthedShop.definition)
    switch simple {
  ...
  let make = (~shopifyAuthed: Types.shopifyAuthed) =>
    <ShopifyPolaris.Provider>
      <ShopifyBridge.Provider
        config={
          open ShopifyBridge
          config(~apiKey=shopifyAuthed.apiKey, ~shopOrigin=shopifyAuthed.shop, ())
        }>
        <ShopifyBridge.Loading /> <ShopifyPolaris.Card /> <Inner />
      </ShopifyBridge.Provider>
    </ShopifyPolaris.Provider>
}
...
let make = () => {
  ...
  | NotAsked | Loading => <Loader.Screen />
